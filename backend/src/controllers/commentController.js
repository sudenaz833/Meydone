...
/** Mekan sahibi yalnızca kendi mekanındaki yorumda beğeni / yanıt kullanabilir. */
async function assertOwnerSocialAccessOnComment(user, comment) {
  if (String(user?.role ?? '') !== 'owner') return;
  const venue = await Venue.findById(comment.venue).select('owner').lean();
  if (!venue) {
    throw new AppError('Venue not found', 404);
  }
  const ownerId = venue.owner != null ? String(venue.owner) : '';
  if (!ownerId || ownerId !== String(user._id)) {
    throw new AppError(
      'Mekan sahibi olarak yalnızca kendi işletmenize ait yorumlarda beğeni verebilir veya yanıt yazabilirsiniz.',
      403,
    );
  }
}
...
export const updateComment = asyncHandler(async (req, res) => {
  ...
  const { rating, text, content, photoUrl } = req.body;
  if (rating !== undefined) comment.rating = Number(rating);
  if (text !== undefined) comment.text = String(text).trim();
  else if (content !== undefined) comment.text = String(content).trim();
...
export const likeComment = asyncHandler(async (req, res) => {
  ...
  await assertOwnerSocialAccessOnComment(req.user, comment);
...
export const addCommentPhoto = asyncHandler(async (req, res) => {
  ...
  if (!url) {
    throw new AppError('file (multipart) or url is required', 400);
  }
...
export const updateReplyInComment = asyncHandler(async (req, res) => {
  const comment = await Comment.findById(req.params.id);
  if (!comment) {
    throw new AppError('Comment not found', 404);
  }
...
export const deleteReplyFromComment = asyncHandler(async (req, res) => {
  const comment = await Comment.findById(req.params.id);
  if (!comment) {
    throw new AppError('Comment not found', 404);
  }
