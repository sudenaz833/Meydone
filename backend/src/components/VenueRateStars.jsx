...
export default function VenueRateStars({ venueId, onRated }) {
  ...
  async function submit(stars) {
    ...
    setBusy(true);
    try {
      const { data } = await api.post(`/api/venues/${venueId}/rate`, {
        rating: stars,
      });
  ...
  const activeLevel = hover;

  return (
    <div className="mt-8 border-t border-sky-100/60 pt-8">
      <h3 className="text-sm font-semibold text-stone-800">Bu mekanı puanlayın</h3>
      <p className={`mt-2 ${textSmall}`}>1–5 yıldız seçin. İstediğiniz zaman yeniden puanlayıp değiştirebilirsiniz.</p>
