...
function commentDocs(venues, user1Id, adminId) {
  return [
    { venue: venues[0]._id, user: user1Id, rating: 5, text: 'Harika meze ve manzara. Kesinlikle tekrar geliriz.' },
    { venue: venues[0]._id, user: adminId, rating: 4, text: 'Etler yumuşak, porsiyonlar doyurucu.' },
    { venue: venues[1]._id, user: user1Id, rating: 5, text: 'Kahve çok dengeli; terasta oturmak keyifli.' },
    { venue: venues[2]._id, user: adminId, rating: 4, text: 'Hızlı servis, gece geç saatte kurtarıcı.' },
    ...
    { venue: venues[5]._id, user: user1Id, rating: 5, text: 'Galata manzarası eşliğinde harika espresso.' },
...
export async function ensureSampleData() {
  ...
  async function ensureMissingSeedVenues() {
    ...
    const docs = missing.map((doc) => ({ ...doc, owner: owner?._id ?? null }));
